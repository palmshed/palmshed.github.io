...
export default async function handler(req) {
  ...
  if (!token || !password) return json({ error: "Missing fields" }, 400, origin);
  if (password.length < 8) return json({ error: "Password too short" }, 400, origin);
  ...
  if (!reset || reset.expires_at < new Date()) {
    return json({ error: "Invalid or expired token" }, 400, origin);
  }
